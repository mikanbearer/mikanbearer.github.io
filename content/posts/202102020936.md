...
isCJKLanguage: true
---
使用command line操作7-Zip壓縮檔案，工作上可能會用到所以紀錄一下
<!--more-->

接下來用PowerShell操作，安裝使用預設目錄，使用call operator `'&'`來呼叫7z.exe
```
...
function doCompression {
    ...
    process {
        try {
            $( Get-Date -Format "yyyy/dd/MM HH:mm K" ) + "  Starting process..." | Out-File -Append $LOG_PATH
            if( Test-Path $ARCHIVE_PATH ) {
                $( Get-Date -Format "yyyy/dd/MM HH:mm K" ) + "  Target archive `"$ARCHIVE_PATH`" is exist, open archive to update." | Out-File -Append $LOG_PATH
                $result = & $7ZIP_EXE u -tzip $ARCHIVE_PATH $SOURCES $("-p" + $PASS)
            }else {
                $( Get-Date -Format "yyyy/dd/MM HH:mm K" ) + "  Target archive `"$ARCHIVE_PATH`" is not exist, create new archive." | Out-File -Append $LOG_PATH
                $result = & $7ZIP_EXE a -tzip $ARCHIVE_PATH $SOURCES $("-p" + $PASS)
            }
	
	        $( Get-Date -Format "yyyy/dd/MM HH:mm K" ) + " " + $result | Out-File -Append $LOG_PATH
        }
        catch {
	        $( Get-Date -Format "yyyy/dd/MM HH:mm K" ) + "  An error has occurred." | Out-File -Append $LOG_PATH
        }
    }
}
